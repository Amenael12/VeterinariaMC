...
<body>
    ...
    <form action="ActualizarMedico.php" method="POST">
        <div class="container">
            ...
            $result = mysqli_query($mysqli,"SELECT * FROM tb_medicos WHERE id_medico =$id");
            while($row = mysqli_fetch_array($result)){
                echo"<input type='hidden' name='id' value='{$row['id_medico']}' required>";
                echo"<input type='text' name='nombre' value='{$row['nombre']}' required>";
                echo"<input type='text' name='apellido' value='{$row['apellido']}' required>";
                echo"<input type='text' name='especialidad' value='{$row['especialidad']}' required>";
